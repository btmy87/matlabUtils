...
%% Example 13, Two Separate Regions
figure(Name="Example 13 - Two Separate Regions")
ha = axes;hold on;
title("Example 13 - Two Separate Regions")
axis equal;
ha.XLim = [-1.2, 1.2];
ha.YLim = [-1.2, 1.2];
theta = linspace(0, 360, 361);
x = [0.4*cosd(theta)-0.5, nan, 0.4*cosd(theta)+0.5];
y = [0.4*sind(theta)-0.5, nan, 0.4*sind(theta)+0.5];
hatch(x, y);